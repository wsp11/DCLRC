...
class y_dequantize(nn.Module):
    ...
    def forward(self, image,quantization):
        y_table = quantization[0]
        return image * y_table
...
class c_dequantize(nn.Module):
    ...
    def forward(self, image,quantization):
        c_table = quantization[1]
        return image * c_table
...
class decompress_jpeg(nn.Module):
    ...
    def forward(self, y, cb, cr, quantization):
        ...
        for k in components.keys():
            if k in ('cb', 'cr'):
                comp = self.c_dequantize(components[k],quantization)
                height, width = int(self.height/2), int(self.width/2)                
            else:
                comp = self.y_dequantize(components[k],quantization)
