...
class StereoImageDataset(Dataset):
    ...
    def compressTo_coefficents(path):
        img = Image.open(path)
        im = to_tensor(img)
        ...
        # new_tensor_Cb = new_tensor_Cb.squeeze(0)
        # new_tensor_Cr = new_tensor_Cr.squeeze(0)

        return new_tensor, new_tensor_Cb, new_tensor_Cr, dimensions, quantization

    def __getitem__(self, index):
        #self.index_count += 1
        im_left = to_tensor(Image.open(self.left_image_list[index]))
        im_left = TF.center_crop(im_left, (370, 740))
        im_left = TF.resize(im_left, (128,256))
        im_right = Image.open(self.right_image_list[index])
        im_right = TF.center_crop(im_right, (370, 740))
        im_right = TF.resize(im_right, (128, 256))
        dimensions_left, quantization_left, Y_coefficients_left, CbCr_coefficients_left = torchjpeg.codec.quantize_at_quality(im_left,75)
        ori_left = torchjpeg.codec.reconstruct_full_image(Y_coefficients_left, quantization_left, CbCr_coefficients_left, dimensions_left)
        dimensions_right, quantization_right, Y_coefficients_right, CbCr_coefficients_right = torchjpeg.codec.quantize_at_quality(im_left, 75)
        ori_right = torchjpeg.codec.reconstruct_full_image(Y_coefficients_right, quantization_right,CbCr_coefficients_right, dimensions_right)
        return Y_coefficients_left,CbCr_coefficients_left,Y_coefficients_right,CbCr_coefficients_right,ori_left,ori_right
